...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        if x[0] < self.statespace_lo[0] or x[0] > self.statespace_hi[0] or x[1] < self.statespace_lo[0] or x[1] > self.statespace_hi[0]:
            return False
        else:
            grid_occupancy = DetOccupancyGrid2D(self.occupancy.width, self.occupancy.height, self.occupancy.obstacles)
            return grid_occupancy.is_free(x)
    ...
    def get_neighbors(self, x):
        ...
        moves = [[l,0],
            ...
            [-l,0],
            [0,-l],
            [np.sqrt(0.5*l**2),np.sqrt(0.5*l**2)],
            [-np.sqrt(0.5*l**2),-np.sqrt(0.5*l**2)],
            [-np.sqrt(0.5*l**2),np.sqrt(0.5*l**2)],
            [np.sqrt(0.5*l**2),-np.sqrt(0.5*l**2)]]
        
        for i in range(0,len(moves)):
            new_x = [x[0] + moves[i][0], x[1] + moves[i][1]]
            new_x = self.snap_to_grid(new_x)
            if self.is_free(new_x) == True:
